...
while getopts ":t:c:p:n:m:s:i" opt; do
    case $opt in
    t)
        target=$OPTARG
        ;;
...
# Delete IDP if one exists
uaac target "$target" --skip-ssl-validation
uaac token client get "$admin_client" -s "$admin_client_secret"

IDP_ID=$(uaac curl /saml/service-providers --insecure \
    | grep -B5 -A5 "\"name\": \"$origin_name\"" \
    | awk '/"id":/{ print substr($2,2,length($2)-3) }')

[[ -z $IDP_ID ]] || uaac curl -X DELETE /saml/service-providers/$IDP_ID --insecure
